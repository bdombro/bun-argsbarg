#!/usr/bin/env bun
/**
 * Merge argsbarg's code.mdc template into a consumer repo, preserving
 * app-specific convention blocks from the existing rule file.
 *
 * Usage: bun scripts/merge-code-rule.ts <consumer-dir> [template-path]
 */
...
if (!consumerDir) {
  console.error("Usage: bun scripts/merge-code-rule.ts <consumer-dir> [template-path]");
  process.exit(1);
}

const templatePath = process.argv[3] ?? join(repoRoot, "examples/full-example-json/.cursor/rules/code.mdc");
const rulePath = join(consumerDir, ".cursor/rules/code.mdc");
