...
test("docs mcp when MCP enabled", async () => {
  ...
  expect(result.stdout).toContain("configure install");
});
...
test("skipsRequiredAppConfigExit includes docs and config builtins", () => {
  ...
  expect(skipsRequiredAppConfigExit(["configure", "install"], caps)).toBe(true);
  expect(skipsRequiredAppConfigExit(["configure", "uninstall"], caps)).toBe(true);
  expect(skipsRequiredAppConfigExit(["configure", "status"], caps)).toBe(true);
...
test("generateMcpGuide includes schema URI and .agents install", () => {
  ...
  expect(guide).toContain("configure install");
